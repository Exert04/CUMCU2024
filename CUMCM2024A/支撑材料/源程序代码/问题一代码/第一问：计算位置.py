...
def velocitycal(velupper,theta,thetaplus1):   # 注意plus1意味着向龙尾靠近1
    ...
    ki = np.array((xiplus1-xi)/(np.sqrt(np.square(xiplus1-xi)+np.square(yiplus1-yi))),(yiplus1-yi)/(np.sqrt(np.square(xiplus1-xi)+np.square(yiplus1-yi))))
    viplus1 = np.array((np.cos(thetaplus1-thetaplus1*np.sin(thetaplus1)))/np.sqrt(np.square(np.cos(thetaplus1)-thetaplus1*np.sin(thetaplus1))+np.square(np.sin(thetaplus1)+thetaplus1*np.cos(thetaplus1))),(np.sin(thetaplus1)+thetaplus1*np.cos(thetaplus1))/np.sqrt(np.square(np.cos(thetaplus1)-thetaplus1*np.sin(thetaplus1))+np.square(np.sin(thetaplus1)+thetaplus1*np.cos(thetaplus1))))
    vi = np.array((np.cos(theta-theta*np.sin(theta)))/np.sqrt(np.square(np.cos(theta)-theta*np.sin(theta))+np.square(np.sin(theta)+theta*np.cos(theta))),(np.sin(theta)+theta*np.cos(theta))/np.sqrt(np.square(np.cos(theta)-theta*np.sin(theta))+np.square(np.sin(theta)+theta*np.cos(theta))))
...
if __name__ == '__main__':
    ...
    for i in range(301):  # 计算300个点
        ...
        for j in range(len(tail)):
            saveform.append(np.format_float_scientific(angtoxy(tail[j],rot(tail[j],b))[0],precision=6))
            saveform.append(np.format_float_scientific(angtoxy(tail[j],rot(tail[j],b))[1],precision=6))

        #saved1ct = {"横坐标x (m)":saveformx,"纵坐标y (m)":saveformy}
        #df = pd.DataFrame(saved1ct)
        #df.to_excel(writer, sheet_name='Sheet1', startcol=1, header=1,index=False)
        #writer._save()
    ...
    df.to_excel(writer, sheet_name='位置', startcol=1, header=1,index=False)
    #print(bodyx, bodyy)
    writer._save()
    writer.close()
    # print(tail)
