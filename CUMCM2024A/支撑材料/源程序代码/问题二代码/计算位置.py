...
if __name__ == '__main__':
    ...
    for i in range(300):  # 计算300个点
        ...
        for j in range(len(tail)):
            ...
            saveformx[j] = np.format_float_scientific(saveformx[j],precision=6)
            saveformy[j] = np.format_float_scientific(saveformy[j], precision=6)
    ...
    #saveformall = np.format_float_scientific(saveformall,precision=6)
    # 利用字典生成式生成表内容并保存
